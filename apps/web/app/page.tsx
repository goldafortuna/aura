...
const highlights = [
  {
    ...
    /** Jaminan kerahasiaan — tidak menjanjikan perilaku vendor di luar kendali kita, tapi menjelaskan arsitektur umum */
    privacyNote:
...
export default function Home() {
  return (
    <main className="relative flex h-screen w-full flex-col overflow-hidden bg-[#f8f7ff]">
      ...
      {/* ── Background decoration (animasi ringan) ─────────────────────────── */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {/* Large soft blobs — drift halus */}
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 pb-4">
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          {highlights.map((h) => {
            const Icon = h.icon;
            const wide = 'wide' in h && h.wide;
            const privacyNote = 'privacyNote' in h ? h.privacyNote : undefined;
            return (
              <div
                key={h.title}
                className={`flex items-start gap-2.5 rounded-xl border border-white/70 bg-white/70 px-4 py-2.5 shadow-sm backdrop-blur-sm ${wide ? 'max-w-[min(100%,22rem)]' : ''}`}
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className={`text-[10px] leading-snug text-gray-500 ${wide ? 'max-w-none' : 'max-w-[180px]'}`}>
                    {h.desc}
                  </p>
                  {privacyNote ? (
